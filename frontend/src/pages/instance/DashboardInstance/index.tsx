...
import { Copy, Globe, LogOut, Power, QrCode, RefreshCw, Send, ShieldAlert, Smartphone } from "lucide-react";
...
function getProxyFailureMessage(reason: string | null | undefined, fallback: string) {
  if (!reason) return fallback;

  const knownReasons: Record<string, string> = {
    proxy_auth_407: "Autenticação do proxy recusada",
    proxy_payment_402: "Conta do proxy sem saldo ou limite excedido",
    proxy_timeout: "Timeout ao conectar pelo proxy",
    proxy_config_incomplete: "Configuração do proxy incompleta",
    proxy_failed: "Falha no teste de conectividade do proxy",
  };

  return knownReasons[reason] ? `${knownReasons[reason]} (${reason})` : reason;
}
...
import { EventsSummaryPanel } from "./EventsSummaryPanel";
import { InstanceEventsPanel } from "./InstanceEventsPanel";
import { MobileSecurityAlerts } from "./MobileSecurityAlerts";
import { OperationalStatsGrid } from "./OperationalStatsGrid";
import { TechnicalDiagnosticsPanel } from "./TechnicalDiagnosticsPanel";
...
function DashboardInstance() {
  ...
  const proxyFailureReason = instance.operational?.proxyHealth?.reason || instance.proxyError;
  const proxyFailureMessage = getProxyFailureMessage(
    proxyFailureReason,
    t("proxy.alert.failed.description", { defaultValue: "Não foi possível estabelecer conexão através do proxy configurado. Por favor, verifique as configurações." }),
  );
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex flex-col gap-6">
        ...
        <MobileSecurityAlerts
          registrationCode={regCodeAlert}
          takeover={takeoverAlert}
          onDismissRegistrationCode={() => setRegCodeAlert(null)}
          onDismissTakeover={() => setTakeoverAlert(null)}
        />
        ...
        <Tabs defaultValue="overview" className="w-full">
          <TabsList className="grid w-full grid-cols-2 sm:w-[360px]">
            <TabsTrigger value="overview">
              {t("instance.dashboard.tabs.overview", { defaultValue: "Visão geral" })}
            </TabsTrigger>
            <TabsTrigger value="diagnostics">
              {t("instance.dashboard.tabs.diagnostics", { defaultValue: "Diagnóstico" })}
            </TabsTrigger>
          </TabsList>

          <TabsContent value="overview" className="mt-6 space-y-6">
            <OperationalStatsGrid instance={instance} numberFormatter={numberFormatter} />

            <EventsSummaryPanel instanceName={instance.name} numberFormatter={numberFormatter} />

            <InstanceEventsPanel instanceName={instance.name} />
          </TabsContent>
