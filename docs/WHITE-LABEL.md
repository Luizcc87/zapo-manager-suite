...
2. **`frontend/src/hooks/useAppConfig.ts`**: Hook React que unifica as configurações locais com as informações dinâmicas retornadas pelo backend (`GET /`), calculando a tag de versão padronizada (`v1.6.17 (Zapo: v1.6.2)`).
...
| `VITE_APP_VERSION` | `1.6.17` | Versão da release oficial da suíte. |
...
| Variável | Padrão | Descrição |
|---|---|---|
| `APP_VERSION` | `1.6.17` | Versão exposta em `GET /` e `GET /runtime/status`. |
