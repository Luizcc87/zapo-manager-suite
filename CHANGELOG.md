...
- **Fix 3 (causa raiz)**: `InstanceSidebar` lia `collapsed` via `useContext(SidebarCollapsedContext)` no próprio corpo do componente, mas o `Provider` só existe dentro de `SidebarShell` — que é filho de `InstanceSidebar`, não pai. Resultado: `collapsed` sempre `false` ali (valor default do contexto), então o `.map()` dos grupos sempre caía no ramo `Collapsible` antigo (texto+ícone expandido inline), mesmo com a coluna renderizada em `w-14` — texto vazando sobre a lista de conversas. Corrigido: `useSidebarCollapsed()` chamado direto em `MainSidebar`/`InstanceSidebar` (fonte única), `collapsed`/`toggle` passados como prop pro `SidebarShell` em vez de o componente gerenciar seu próprio state interno duplicado. Confirmado via `a11y snapshot` (`button "Configurations" haspopup="menu"` → click → `menu "Configurations"` com `menuitem`s, sem texto solto).
