...
test('classifyOtpRegistrationError detects blocked OTP object responses', () => {
  const error = classifyOtpRegistrationError({
    custom_block_screen: { title: 'Login not available right now' },
    login: '555596773757',
    reason: 'blocked',
    status: 'fail',
  });
  ...
  assert.equal(error.statusCode, 423);
  assert.equal(error.code, 'otp_blocked');
  assert.match(error.details || '', /custom_block_screen/);
});
