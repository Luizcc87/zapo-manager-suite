...
2. Execute o script definindo a tag (ou use `latest` por padrão):
   ...
   ```
3. **Criação de Release no GitHub (Obrigatório / Perguntar):**
   Ao criar e enviar uma nova tag Git (`git tag vX.Y.Z && git push origin master vX.Y.Z`), o agente/desenvolvedor deve obrigatoriamente criar ou solicitar confirmação para criar a Release no GitHub via `gh CLI` ou interface:
   ```powershell
   # Em ambiente Windows com GITHUB_TOKEN local conflitante:
   $env:GITHUB_TOKEN=$null; gh release create vX.Y.Z --title "vX.Y.Z — Título" --notes "Notas da release"
   ```
4. Consulte as instruções detalhadas em [docs/DOCKER.md](file:///d:/Projetos%20Dev/Outros/apis-whatsapp-doc-testes/zapo-manager/docs/DOCKER.md) e [docs/SYNC-UPSTREAM.md](file:///d:/Projetos%20Dev/Outros/apis-whatsapp-doc-testes/zapo-manager/docs/SYNC-UPSTREAM.md).

---

## 🛠️ Metodologia BMAD (BMAD Method v6.10.0)

O projeto utiliza a metodologia **BMAD (Build More, Architect Dreams)** para especificação, arquitetura, design e testes. 

*   **Executor de Scripts (`uv run`):** Todos os scripts Python do BMAD e customizações devem ser executados usando `uv run <script>` em vez de `python3` diretamente. O suporte a `python3` direto será removido na v7.
